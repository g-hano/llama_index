...
class HuggingFaceMultiModal(MultiModalLLM):
    ...
    def stream_complete(
        self, prompt: str, image_documents: Sequence[ImageDocument], formatted: bool = False, **kwargs: Any
    ) -> CompletionResponseGen:
        """Streaming completion endpoint for multimodal input."""
        prepared_inputs = self._prepare_messages(
            [ChatMessage(role="user", content=prompt)], image_documents
        )
...
class LlamaMultiModal(HuggingFaceMultiModal):
    ...
    def _prepare_messages(
        ...
        """
        images = [Image.open(image.image_path) for image in image_documents]
        messages = [
            {
                "role": "user", 
                "content": [
        ...
        inputs = self._processor(images, input_text, add_special_tokens=False, return_tensors="pt").to(self._model.device)
        return inputs
